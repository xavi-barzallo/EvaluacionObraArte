package ec.ups.edu.eva.EVA_Barzallo_Xavier_SRV.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        File f = new File("./target/EVA_Barzallo_Xavier_SRV.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "EVA_Barzallo_Xavier_SRV.war").importFrom(f).as(WebArchive.class);
        
        // Add the package containing the test classes:
        war.addPackage("ec.ups.edu.eva.EVA_Barzallo_Xavier_SRV.test");
